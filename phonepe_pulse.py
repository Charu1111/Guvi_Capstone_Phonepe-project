...
conn = mysql.connector.connect(user='root', password='Charu333', host='127.0.0.1', database='phonepe_pulse')
...
if SELECT == "Home":
    ...
    col1.image(Image.open("D:\\Guvi_project\phonepe\\Phonepe_image2.png"),width = 300)
    with col1:
    ...
    with col2:
        st.video("D:\Guvi_project\phonepe\pulse-video.mp4")
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col1:
            ...
            df2: DataFrame = pd.read_csv(r"D:\\Guvi_project\\phonepe\\Longitude_Latitude_State_Table.csv")
            df1.State = 'df2'
        ...
        with col2:
            ...
            cursor.execute(f"select state, sum(count) as Total_Transactions, sum(amount) as Total_amount from map_trans where year = {Year} and quarter = {Quarter} group by state order by state")
            df1 = pd.DataFrame(cursor.fetchall(),columns= ['State', 'Total_Transactions', 'Total_amount'])
            df2 = pd.read_csv(r"D:\\Guvi_project\\phonepe\\Statenames.csv")
...
if SELECT == "About":
    ...
    with col1:
        st.video("D:\Guvi_project\phonepe\pulse-video.mp4")
    with col2:
        st.image(Image.open("D:\\Guvi_project\\phonepe\\Phonepe_image2.png"),width = 500)
        st.write("---")
    ...
    with col1:
        ...
        st.image(Image.open("D:\\Guvi_project\\phonepe\\about_phonepe.jpg"),width = 400)
        with open("D:\\Guvi_project\\phonepe\\about_phonepe1.png","rb") as f:
            data = f.read()
        st.download_button("DOWNLOAD REPORT",data,file_name="annual report.pdf")
    with col2:
        st.image(Image.open("D:\\Guvi_project\\phonepe\\about_phonepe1.png"),width = 800)
